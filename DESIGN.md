...
| 34 | Mobile renders decision 33's route-aware chat as a persistent **draggable bubble** launcher + a **non-modal, resizable half-height panel** (no scrim — content behind stays scrollable/tappable) | Phones have no `⌘/Ctrl-J` and no room for a side panel; a movable bubble is the touch analog of the shell toggle, and a barrier-free panel is what lets you read and scroll the prose you're fixing *while* you chat. Same contract and context-follows-route as web — only the presentation differs |
| 35 | **Mostly flat, low-shadow visual language** across both clients (extends decisions 11, 21): no drop shadows on static surfaces; hierarchy via hairline borders + tonal surface tints; a subtle shadow kept only on floating transient overlays (dialogs, menus, ⌘K palette, AI panel/bubble) | Stock Material over-uses float shadows; the warm editorial "book page, not admin panel" identity (`:506`) reads better flat. On Flutter this is component `elevation: 0` + Material 3 tonal elevation (`surfaceContainer*`, `surfaceTintColor`) and `outlineVariant` borders; on web, borders + container-tone via CSS variables. Reserving shadow for genuinely floating affordances keeps elevation meaningful instead of decorative |
