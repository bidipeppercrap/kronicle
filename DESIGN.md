...
> Written 2026-06-09. Updated 2026-06-10: renamed to Kronicle, design review fixes (immutable IDs, status column, auth proxy, quick capture, build phases); added Web UI Design (typography + warm editorial theme); added writing-tool features (backlinks, editor spec, backups, AI ground rules, revisions, era validation, server-side slug rename); added AI chat (per-entity tool-calling chat, approval-gated writes); added Mobile UI Design (Material 3 warm theme + bundled OFL fonts); added Cloudflare Access in front of the web app + implementation defaults (monorepo layout, server-side slug generation, partial PUT, list envelope, revision endpoints); built the Phase 1 web app (`web/`) — component layer is Bits UI directly + hand-styled Tailwind rather than shadcn-svelte (decision 27); built per-entity AI chat from Phase 3 — Worker `POST /api/ai/chat` (DeepSeek tool loop, SSE, read tools capped at 8/turn, write tools intercepted as proposals) + web `AiChatPanel` on detail/edit views (diff and change-card proposals with Apply/Discard; in the editor, applied edits merge into the open buffer and ride autosave). Updated 2026-06-11: considered and rejected an app-level login (first-run root user + TOTP QR) — Cloudflare Access stays the sole web gate; added its setup runbook to the Auth section. Editor gains a Write/Peek toggle (Ctrl+E) that swaps the editing surface with rendered Markdown in place — a side-by-side preview pane was rejected for screen cost. Built revision history from Phase 3: `entity_revisions` table, snapshots coalesced to a 10-minute window, list/restore endpoints, History section in the editor sidebar. Added free-form tags (`metadata.tags` + `?tag=` filter + chips in editor/detail/list), per-type content templates (full create form + blank-editor affordance; stubs stay bare), family relationship types (`parent_of`, `sibling_of`, `married_to`), and a vault-health report (`GET /api/diagnostics` + web `/health`: broken wikilinks, orphans, stale stubs — computed at read time like backlinks). Moved the AI provider key into a D1 `settings` table editable from a new web Settings page (`GET/PUT /api/settings/ai`, masked hint only, env secret stays as fallback — decision 31); fixed chat self-applying its own proposals by blocking same-turn `apply_proposal` on both Worker and client (decision 32); documented that the AI provider is any OpenAI-compatible chat-completions endpoint (OpenAI, OpenRouter, Claude compat, Gemini compat, Ollama) configured entirely from Settings — DeepSeek is only the default. Updated 2026-06-13: hardened per-entity chat against the model narrating a change instead of calling a write tool — the system prompt now states that only a tool call renders a proposal card and that the `[Proposed …]` history lines are app records it must never type itself (the failure surfaced as a bare `[Proposed — awaiting approval]` message with no Apply button). Replaced the per-entity (Phase 3) + planned vault-wide (Phase 4) chats with one **route-aware chat** — a single shell-mounted panel whose context auto-follows the route (the entity you're viewing → a timeline era → the whole vault), `entity_id` auto-set each turn with a flattened `[Context: …]` marker when focus changes, the vault index always in the system prompt, and proposals applied via REST + refetch (merging into the open editor buffer only when they target the entity being edited); the worker dropped its `entity_id`-required guard and the web `AiChatPanel` moved out of the detail/editor pages into the shell (toggle + `⌘/Ctrl-J`) — decision 33. Built the **Timeline** (worker `GET /api/timeline?era=&status=` → events sorted by `metadata.order_index` ascending, position-less ones last, plus the era `lore` entities for band labels; web `/timeline` route + sidebar nav, a chronological feed that draws era headers over the stream and falls back to a flat stream when no eras exist). Clarified that `era` is **optional** — the timeline is chronological-first and eras are opt-in grouping bands, not a required section (the validator already passes an absent/null `era`); the AI chat system prompt now carries the event/timeline metadata conventions (`order_index`/`date`/`precision`/optional `era`, plus tags and per-type fields) so it proposes fields that fit instead of inventing them. Built **Export/Import** from Phase 3: `GET /api/export` streams a zip (`<type>/<slug>.md` per entity with YAML frontmatter, a lossless `kronicle.json` manifest, and every media file from R2; the AI key is never included), `POST /api/import` restores it (driven solely by `kronicle.json`, so no YAML parser) — guarded against clobbering by refusing a non-empty vault unless `?replace=true`, which wipes D1 + R2 first; web `/export` page ("Backup" nav) with a download link and a replace-aware import form. Uses `fflate` for zip in the Worker; the weekly cron-to-R2 backup remains unbuilt. Revised the **mobile AI chat** presentation (Phase 4, unbuilt): replaced the planned modal bottom sheet with an app-wide **draggable chat bubble** launcher (coexists with the quick-capture FAB on Home) opening a **non-modal, resizable half-height floating panel** with no scrim, so the screen behind stays scrollable while chatting — the touch idiom for web's route-aware shell panel (decision 33), recorded as decision 34. Same `POST /api/ai/chat` contract and context-follows-route behavior; presentation only. Adopted a **mostly-flat, low-shadow visual language** for both clients (decision 35): stock Material's drop shadows are suppressed on static surfaces — separation comes from hairline borders + tonal surface tints (Flutter: component `elevation: 0` + Material 3 tonal elevation / `outlineVariant`; web: borders + container-tone via CSS variables) — with a subtle shadow reserved only for floating transient overlays (dialogs, menus, ⌘K palette, AI panel/bubble). Built the **Flutter Android client** (`mobile/`) covering **Phase 2 and Phase 4** in one pass: quick capture (one-step name + optional note → `stub`), the four browse tabs (Home dashboard with stubs/recent/stats, entity List with type tabs + status filter + inline search, chronological Timeline with era bands drawn over the stream, full-text Search grouped by type), entity Detail (metadata, Markdown with tappable `[[wikilink]]` navigation, media gallery via auth-headered `Image.network`, relationships both directions, children, backlinks, inline status promotion + delete), and a basic Editor (name/type/status/summary/Markdown field in Quattro, tags, parent + relationship pickers). Online-first with the in-memory URL-keyed response cache from the caching table (write clears all). The route-aware chat (decision 33) is realized as the shell overlay from decision 34 — an app-wide draggable bubble opening a non-modal, resizable floating panel with no scrim, mounted above every route via `MaterialApp.builder`; context follows a focus stack (detail/editor push their entity, popping restores), SSE `text`/`reading`/`proposal` events render the same diff/change cards, Apply dispatches through the REST endpoints (or merges into the open editor buffer via an editor bridge) and bumps a refresh bus so on-screen views refetch — the same `{entity_id?, messages}` contract and `[Context: …]`/`[Proposed …]` flattened history as web. The three fonts (Literata, Inter, iA Writer Quattro) are bundled APK assets, not `google_fonts` (decision-consistent: no Google requests); prose renders via `flutter_markdown`. The APK embeds the bearer token (accepted per Auth); URL + token come from `--dart-define` (defaults: emulator `10.0.2.2:8787`, `dev-token`). Mobile **Graph** and **Export** screens remain unbuilt (Phase 3 scope, not Phase 2/4). Updated 2026-06-14: added a ninth entity type, **`creature`** (flora & fauna — plants, animals, beasts), promoting creatures out of `lore` into a first-class type with its own icon (PawPrint), metadata convention (`category` flora/fauna, `habitat`, `diet`, `danger`), and content template; additive across worker/web/mobile type lists — no migration, since `type` has no SQL CHECK (decision 36). Added a **Clear** control to the route-aware chat header on both clients (web `AiChatPanel` eraser button, Flutter `ChatPanel` sweep button, each shown only once the transcript has turns and disabled mid-stream): the chat persists across open/close (web component stays shell-mounted; Flutter `ChatController` is a singleton), so Clear is the one-tap reset that wipes the transcript, context markers, fetched baselines, and any manual whole-vault detach back to a blank slate — purely client state, nothing written server-side. Backstopped the chat against the model faking a proposal in prose (the 2026-06-13 prompt rule alone still let a literal `[Proposed p_xxx: … — applied]` line slip through as text every so often, leaving the writer a fake "applied" with no Apply button): both clients now deterministically strip the app-record markers (`[Proposed …]`, `[Context: …]`) from assistant text — in the rendered bubble **and** in the history sent back next turn (so the hallucination can't reinforce itself). Real proposals are unaffected since they arrive as `proposal` SSE events and render as cards, never as text; the `[Context: …]` prefix on user turns is untouched. Replaced the default Flutter launcher icon with the crystal-ball brand mark (gold on espresso, rendered from the web favicon to the five mipmap densities). Reworked **mobile auth** so the APK can be distributed: the Flutter client no longer embeds the bearer token — server URL + token are entered in-app (a new Settings screen, also shown as first-run onboarding when unconfigured) and held in `flutter_secure_storage` (encrypted at rest, Android Keystore-backed; `minSdk` raised to 23); `--dart-define` URL/token survive only as dev seeds that pre-fill an unconfigured install, and Settings save verifies the credentials against the Worker before letting a first-run user through (decision 37, supersedes the embed-the-token note in the Auth section). Gave the mobile chat bubble **Messenger-style edge anchoring** — free-drag while held (fixed a lag where fast drags trailed the finger because move deltas were accumulated onto a stale build-time offset), then an eased fly-to-the-nearer-side-edge snap on release (decision 34); and let the two genuinely-floating controls cast a soft shadow — the FAB now carries a real `elevation` and the bubble a deeper `boxShadow` — refining decision 35's flat language to reserve shadow for things that actually hover over content. Also added the `INTERNET` permission to the release Android manifest (Flutter only injects it into the debug manifest), which had left distributed/release APKs unable to reach the Worker (`Failed host lookup`).
...
| `creature` | Flora and fauna — plants, animals, beasts (a `category` field splits flora vs fauna) |
| `location` | Towns, cities, forests, regions, buildings |
| `faction` | Groups, guilds, covens, families |
| `lore` | Magic systems, history, religions, artifacts |
...
| `type` | TEXT | `character`, `creature`, `location`, `faction`, `lore`, `story`, `chapter`, `event`, `ability` |
...
**Creature (flora & fauna):**
```json
{ "category": "fauna", "habitat": "deep forest", "diet": "carnivore", "danger": "high" }
```
...
**Stateless and ephemeral.** The client holds the conversation and sends the full message history every turn. The panel persists across open/close (it stays mounted in the shell), so a one-tap **Clear** control in the header is how you start over — it wipes the transcript, the context markers, fetched baselines, and any manual whole-vault detach. No chat tables in D1 — the durable artifacts are the entities themselves (and their revisions), not the chatter that produced them.
...
**The token must never reach the browser.** The SvelteKit app calls the Worker exclusively from its server routes (`+page.server.ts` / `+server.ts`), where the token lives in an environment variable. Shipping it in client-side JS on a public URL would hand out full write access plus the DeepSeek proxy (your API credits). The Flutter app does **not** embed the token: the server URL and token are entered in-app and held in `flutter_secure_storage` (encrypted at rest, Android Keystore), so the APK carries no secret and can be distributed. Each install holds its own token; a fresh install opens onto a connect screen. (`--dart-define` URL/token survive only as dev seeds that pre-fill an unconfigured install.)
...
| _(shell overlay)_ | **Route-aware AI chat** — an app-wide draggable bubble opens a non-modal, resizable floating panel (no scrim, content stays scrollable behind it); its context follows the route (entity → era → whole vault) |
...
**Elevation — mostly flat (shared across both clients).** Stock Material's drop shadows are deliberately suppressed: static surfaces (cards, list rows, app bar, nav, FAB) carry **no shadow**. Hierarchy comes from **hairline (1px) warm-toned borders** where a hard edge helps (inputs, cards, dividers) plus **subtle surface-tone shifts** (a slightly lighter/darker container fill) elsewhere. A soft shadow is kept *only* on genuinely floating, transient overlays — dialogs, dropdown menus, the ⌘K command palette, and the AI chat panel — so "above the page" still reads. This governs separation only; it never adds chrome around prose (see Reading layout rules — the detail page stays borderless and cardless).
...
- A **Clear** button in the header (shown once the transcript has turns) wipes the conversation back to a blank slate — transcript, context markers, fetched baselines, and any whole-vault detach all reset. The panel persists across open/close, so this is the deliberate "start over"; nothing is ever written server-side
...
- Mobile reaches the same route-aware chat through a different idiom (no keyboard for `⌘/Ctrl-J`): an **app-wide draggable chat bubble** floats over every screen and, on Home, coexists with the quick-capture FAB — movable so it never traps the content beneath it. Tapping it opens a **non-modal floating panel** anchored at ~half height with **no dimming scrim**, so the screen behind stays scrollable and tappable — the point is to read and scroll the prose you're fixing *while* you chat about it; a drag handle resizes it (peek / half / near-full). The header context chip, the header Clear control, and context-follows-route behavior (entity → era → whole vault) match web; v1 still renders proposals as a full-replace preview rather than an inline diff

### Mobile UI Design (Flutter)

Same identity as web, native idiom: **Material 3, re-themed to the warm editorial palette**. Material 3 is Flutter's built-in component set — FAB (quick capture), navigation bar, search bar all come for free, with no extra design-system dependency. The AI chat is the one custom piece layered on top: a persistent overlay bubble plus a non-modal, `DraggableScrollableSheet`-style standing panel mounted with **no barrier**, so the content behind stays scrollable while chatting — a stock modal bottom sheet would dim and block it. Cupertino or a custom design system would be the wrong bar for an Android-only personal tool.
...
- **Mostly flat, low shadow** (the same elevation rule as web): static components default to `elevation: 0` — `Card`, `AppBar`, `NavigationBar`, `FAB`, etc. — so Material's drop shadows are stripped. Separation comes from `outlineVariant` hairline borders plus Material 3 **tonal elevation** (the `surfaceContainerLow/High` surface roles and `surfaceTintColor`), not shadow. A subtle shadow is kept only on genuinely floating, transient overlays — dialogs, menus, and the chat bubble + non-modal AI panel (decision 34) — which is what keeps "mostly flat" honest: the one floating affordance still floats
...
**Format:** zip containing one Markdown file per entity (`<type>/<slug>.md`, YAML frontmatter + prose) + `kronicle.json` manifest + all media files. The AI provider key is never exported. Import is driven entirely by `kronicle.json` (the Markdown is human-/Obsidian-facing only), so there is no YAML parser to maintain. Lossless round-trip — `POST /api/import` restores the zip into a fresh D1 + R2 instance; it **refuses a non-empty vault unless `?replace=true`**, which wipes entities/relationships/revisions/media (D1 + R2) before restoring.
...
| 34 | Mobile renders decision 33's route-aware chat as a persistent **draggable bubble** launcher + a **non-modal, resizable half-height panel** (no scrim — content behind stays scrollable/tappable). The bubble **anchors to the nearer side edge on release** (Messenger-style: free-drag while held, eased fly-to-edge on let go) | Phones have no `⌘/Ctrl-J` and no room for a side panel; a movable bubble is the touch analog of the shell toggle, and a barrier-free panel is what lets you read and scroll the prose you're fixing *while* you chat. Edge-snapping keeps it out of the content's way and off the thumb-occluded centre, and matches the bubble idiom users already know. Same contract and context-follows-route as web — only the presentation differs |
| 35 | **Mostly flat, low-shadow visual language** across both clients (extends decisions 11, 21): no drop shadows on static surfaces; hierarchy via hairline borders + tonal surface tints; a soft shadow kept only on genuinely floating affordances — transient overlays (dialogs, menus, ⌘K palette, AI panel) **and** the persistently-floating controls that hover over content (the chat bubble and the mobile FAB) | Stock Material over-uses float shadows; the warm editorial "book page, not admin panel" identity (`:506`) reads better flat. On Flutter this is component `elevation: 0` + Material 3 tonal elevation (`surfaceContainer*`, `surfaceTintColor`) and `outlineVariant` borders; on web, borders + container-tone via CSS variables. The exceptions earn their shadow by actually floating above the page — the FAB gets a real `elevation` and the chat bubble a hand-tuned `boxShadow`; reserving shadow for these keeps elevation meaningful instead of decorative |
| 36 | Flora and fauna are a first-class entity type **`creature`** (ninth type), not folded into `lore`. Metadata convention: `category` (`flora`/`fauna`), `habitat`, `diet`, `danger`; icon PawPrint; the usual heading template | A worldbuilding vault accumulates beasts and plants worth their own browsable type, icon, and timeline-adjacent metadata — burying them in generic `lore` made them unfindable. Purely additive: `type` carries no SQL CHECK (only a Drizzle/zod enum), so adding a value needs no migration, and the `Record<EntityType, …>` client maps make the compiler enforce every new key |
| 37 | The Flutter client does **not** embed the bearer token. Server URL + token are entered in-app (Settings screen, also first-run onboarding) and kept in `flutter_secure_storage` (Android Keystore, encrypted at rest); `--dart-define` URL/token are dev seeds only; `minSdk` raised to 23 for the storage backend | The APK is now distributable — an embedded token would hand every recipient full write + AI-proxy access, and a leaked APK would leak the secret. In-app entry means each install carries its own token, encrypted, and a fresh install onboards instead of silently failing. Supersedes the original "APK embeds the token — accepted risk" note now that distribution is in scope |
